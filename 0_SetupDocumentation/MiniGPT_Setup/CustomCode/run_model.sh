...
if [ "$#" -ne 3 ]; then
    echo "Usage: $0 <llama|vicuna> <OpenPilot_comma_ai|OpenPilot_2k19|OpenPilot_utah> <0|1>"
    exit 1
fi
...
# Set the cfg-path based on user input
if [ "${dataset_choice}" = "OpenPilot_comma_ai" ]; then
    ...
    echo "Invalid input. Please enter 'OpenPilot_comma_ai' or 'OpenPilot_2k19' or 'OpenPilot_utah'."
    exit 1
fi
