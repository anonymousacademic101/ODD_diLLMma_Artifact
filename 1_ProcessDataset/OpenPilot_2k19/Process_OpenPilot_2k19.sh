...
# Declare directorys
INPUT_DIRECTORY="../../0_Datasets/Data/OpenPilot_2k19/0_OriginalData"
OUTPUT_DIRECTORY="../../0_Datasets/Data/OpenPilot_2k19/1_ProcessedData"
...
find "$OUTPUT_DIRECTORY" -name "*.hevc" | while read file; do
...
for i in "$OUTPUT_DIRECTORY"/*.hevc; do
    # ffmpeg -i "$i" -vf setpts=1.25*PTS -an -c:v libx264 -crf 23 "${i%.*}.mp4"
    ffmpeg -i "$i" -vf setpts=1.25*PTS -an -r 15 -c:v libx264 -crf 23 "${i%.*}_15.mp4"
done
