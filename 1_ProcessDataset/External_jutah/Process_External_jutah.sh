...
# Declare directorys
INPUT_DIRECTORY="../../0_Datasets/Data/External_jutah/0_OriginalData/"
OUTPUT_DIRECTORY="../../0_Datasets/Data/External_jutah/1_ProcessedData/"
...
for file in "$INPUT_DIRECTORY"/*.mp4; do
  ...
  if [ -e "$file" ]; then
    ...
    ffmpeg -ss 90 -i "$file" -an -r 15 "$OUTPUT_DIRECTORY$new_filename"
